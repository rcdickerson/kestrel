...
do
  ...
  do
    ...
    do
        ...
        (time timeout 10m $kestrel_exec --infer-invariants --output-summary $summary_file -i $file -o $output_file --output-mode=$out_mode $technique --sa-max-iterations=12000) > "$log_dir/$file_basename".log 2>&1
    done
  done
done
echo "Done"
